...
export const getAccommodations = async (req, res, next) => {
  ...
  try {
    ...
    if (!accommodations) {
      throw new accommodationNotFound(res.__('There are no accommodations available'));
    }
...
export const getOneAccommodation = async (req, res, next) => {
  ...
  try {
    ...
    const amenities = await models.Amenity.findOne({ where: { accommodationID: id }, attributes: { 
      exclude: [
        res.__('createdAt'), 
        res.__('updatedAt')
      ] 
...
export const updateAccommodation = async (req, res, next) => {
  try {
    ...
    res.status(201).json({ status: 201, message: res.__('Accommodation successfully updated') });
  } catch (error) {
    next(error);
  }
};
...
export const bookAccomodation = async (req, res, next) => {
  try {
    ...
    if (accommodations.numberOfRooms === 0) {
      throw new badRequest(res.__('Accommodation is currently full'));
    }
    ...
    res.status(201).json({ message: res.__('Booking successfully made'), data: booking });
  } catch (error) {
    next(error);
  }
};
