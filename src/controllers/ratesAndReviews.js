...
export const createReviews = async (req, res, next) => {
    ...
    try{
        ...
        if(tokenVerify){
            ...
            if(addReview){
                res.status(201).json({status: 201, message:res.__('Review added successfully!'), data: addReview});
            }else{ throw new ApplicationError(res.__('Failed to add a review'), 500)}
    
        }else { throw new authorizationError(res.__(`You're not authenticated! Please login.`))}
    }
    catch(error){
        next(error);
    }
...
export const getReviews = async (req, res, next) => {
    ...
    try{
        ...
        const paginatedReviews = {
            offset: skip,
            limit,
            attributes: [res.__('rate'), res.__('review')]
        ...
        if(findAllRatings){
            if(!findAllRatings.allRatings.count){ throw new notFoundRequestError(res.__("No reviews found, be the first to rate this accommodation!"))}
        ...
        throw new ApplicationError(res.__("Failed to load reviews, try again!"));
    }
    catch(error){
        next(error);
    }
