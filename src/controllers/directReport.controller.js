...
export const approve_reject_TravelRequest = async (req, res, next) => {
  ...
  const decoded = await getDataFromToken(req, res, next);
  try {
    if (action === res.__('approve') || action === res.__('reject')) {
      ...
      if (findTravelRequest) {
        ...
        if (findTravelRequest.managerId === userId) {
          ...
          if (findTravelRequest.status === res.__('pending') || (findTravelRequest.status === res.__('rejected') && action !== 'reject')) {
            ...
            if (updateStatus) {
              ...
              const newNotificantion = {
               ...
               pusher.trigger('bare-foot-normad', res.__('notification'), notification);
